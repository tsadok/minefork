...
local oredefs = {
	...
	mithril = {
        ...
        toolgroups = { mithriltool=1 },
		tools = {
			pick = {
				cracky = {times = {[1] = 1.25, [2] = 0.25, [3] = 0.15}, uses = 300, maxlevel = 2}
			},
			hoe = {
				uses = 1000,
			},
			...
			axe = {
				...
				fleshy = {times = {[2] = 0.95, [3] = 0.30}, uses = 300, maxlevel = 1}
			},
			sword = {
...
if minetest.get_modpath("dungeon_loot") ~= nil then
    dungeon_loot.register({name = "moreores:silver_ingot", chance = 0.2, count = {1,3}})
    dungeon_loot.register({name = "moreores:mithril_ingot", chance = 0.1 })
end

minetest.register_tool("moreores:screwdriver_mithril", {
	description = S("Mithril Screwdriver") .. "\n" .. S("(left-click rotates face, right-click rotates axis)"),
	inventory_image = "screwdriver-mithril.png",
	groups = {tool = 1, mithriltool = 1},
    max_uses = 4500,
	on_use = function(itemstack, user, pointed_thing)
		screwdriver.handler(itemstack, user, pointed_thing, screwdriver.ROTATE_FACE, 4500)
		return itemstack
	end,
	on_place = function(itemstack, user, pointed_thing)
		screwdriver.handler(itemstack, user, pointed_thing, screwdriver.ROTATE_AXIS, 4500)
		return itemstack
	end,
})
minetest.register_craft({
	output = "moreores:screwdriver_mithril",
	recipe = {
