...
function moretrees.grow_beech_or_hallelujah_tree(pos)
    local minaltitude = tonumber(minetest.setting_get("cloudlands_altitude")) or 200
    minetest.log("action", "A sapling grows into a tree at y=" .. pos.y .. " (where cloudlands_altitude=".. minaltitude ..")")
    if (pos.y < minaltitude) then
        --moretrees.grow_beech(pos)
        -- local options = moretrees.beech_model
		...
		-- 			minetest.swap_node(pos, biome_lib.air)
		-- 		else
        --             -- I believe this is the main thing we're trying to do right here:
        ...
        moretrees.grow_hallelujah_tree(pos)
    end
end

function moretrees.grow_hallelujah_tree(pos)
    if (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.crimsontree)
    elseif (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.azuretree)
    elseif (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.springgreentree)
    elseif (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.hotpinktree)
    elseif (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.galiontree)
    elseif (math.random(4) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.blueberrytree)
    elseif (math.random(3) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.yellowtree)
    elseif (math.random(2) == 1) then
         moretrees.grow_custom_tree(pos, moretrees.magentatree)
    else
         moretrees.grow_beech(pos)
    end
end

function moretrees.grow_beech(pos)
    moretrees.grow_custom_tree(pos, moretrees.beech_model)
end

function moretrees.grow_custom_tree(pos, model)
    minetest.swap_node(pos, biome_lib.air)
    minetest.spawn_tree(pos, model)
end
