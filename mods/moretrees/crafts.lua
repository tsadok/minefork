...
-- Finally, some recipes in other mods call for default-tree wood
-- specifically, so if we're replacing that with beech tree planks, we
-- can either track down and fix every single one of those recipes, OR
-- we can just add one recipe that turns beech tree planks into default
-- tree wood.  Guess which one is easier?
minetest.register_craft({
...
-- For completeness, we also do one for the trunks.
minetest.register_craft({
   output = "default:tree 4",
   recipe = {
       {"moretrees:beech_trunk", "moretrees:beech_trunk"},
       {"moretrees:beech_trunk", "moretrees:beech_trunk"},
   }
})
-- And for added bonus completeness, let's go the other way too, just because we can:
minetest.register_craft({
   output = "moretrees:beech_planks 4",
   recipe = {
       {"defaualt:wood", "defaualt:wood"},
       {"defaualt:wood", "defaualt:wood"},
   }
})
minetest.register_craft({
   output = "moretrees:beech_trunk 4",
   recipe = {
