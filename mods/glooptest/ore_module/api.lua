...
function glooptest.ore_module.register_ore(modname, name, desc, uses)
	...
	if uses.tools ~= nil and uses.tools.make ~= nil then
		...
		-- end
        if uses.tools.make.scythe == true then
            local g = { scythe = 1  }
            g[uses.tools.group] = 1
            minetest.register_tool(modname .. ":" .. "scythe_" .. name, {
                description = desc .. " Scythe",
                inventory_image = uses.tools.texture.scythe,
                tool_capabilities = uses.tools.caps.scythe,
                groups = g,
            })
            glooptest.ore_module.set_tool_recipe(modname,"scythe",modname..":"..name.."_ingot",name)
        end
