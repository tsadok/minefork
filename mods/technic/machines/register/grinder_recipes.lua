...
local recipes = {
	...
	{"technic:sulfur_block",       "technic:sulfur_dust 18"},
	{"default:stone",              "technic:stone_dust"},
    {"basic_materials:limestone",  "basic_materials:limestone_dust"},
	{"default:sand",               "technic:silt"},
    {"technic:silt",               "default:clay_lump"},
...
if minetest.get_modpath("gloopores") or minetest.get_modpath("glooptest") then
    ...
    table.insert(recipes, {"gloopores:alatro_block",  "technic:alatro_dust 18"})
	table.insert(recipes, {"gloopores:kalite_block",  "technic:kalite_dust 18"})
    table.insert(recipes, {"gloopores:arol_ingot",    "technic:arol_dust 1"}) -- no block for arol
    table.insert(recipes, {"gloopores:talinite_block","technic:talinite_dust 18"})
    table.insert(recipes, {"gloopores:akalin_block",  "technic:akalin_dust 18"})
end

if minetest.get_modpath("glowstone") then
	table.insert(recipes, {"glowstone:glowore",   "glowstone:glowdust 6"})
end
...
minetest.register_node("technic:silt", {
    description = S("Silt"),
...
minetest.register_node("technic:loam", {
    description = S("Loam"),
    tiles = { "technic_loam.png" },
    groups = { crumbly = 3, },
    sounds = default.node_sound_dirt_defaults(),
})

minetest.register_craft({
    output = "technic:loam 3",
    type   = "shapeless",
    recipe = {"group:sand", "technic:silt", "default:clay_lump"},
})

minetest.register_craft({
    output = "default:dirt 2",
    type   = "shapeless",
    recipe = {"technic:loam", "group:leaves", "group:leaves", "group:leaves" },
})
