...
--- water deco
local rbwt1 = { name = "lowercrossroads:log", param2 = 9, force_place = true, prob = 255 }

-- using default:tree leads to other mods adding decorations to it on world gen;
-- some of those decorations, notably vines, are not suitable for use in water,
-- as they create one-block "holes" in the surface of the water, which looks very
-- very wrong.  So we make our own "log" node, which does not belong to the tree
-- group, so those other mods won't do their things with it; it drops default:tree
-- when broken.  Deciding whether this is a natural log or a common log, is left
-- as an exercise for the apt student.  Either way it's a water log.  HTH.HAND.
minetest.register_node("lowercrossroads:log", {
	description = "Log",
